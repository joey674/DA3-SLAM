...
def main():
    ...
    # 5. 可视化 (使用从VGGT代码中提取的viser_wrapper)
    print("Starting viser visualization...")
    
    # 示例调用 (假设viser_wrapper已定义):
    viser_server = viser_wrapper(
        pred_dict=pred_dict,
        port=8080,                    # 网页服务器端口
        init_conf_threshold=25.0,     # 初始过滤阈值(百分比)
        use_point_map=True,           # 使用计算出的world_points
        background_mode=False,        # 是否后台运行
        mask_sky=False,               # 是否进行天空分割
        image_folder=None             # 如果启用天空分割，需要提供图像文件夹路径
    )
